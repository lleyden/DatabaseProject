...
if(isset($_GET['mode']) && $_GET['mode'] === 'sections' && isset($_GET['course_id'])){
    ...
    $stmt = $conn->prepare("SELECT sec_id, semester, year FROM section WHERE course_id=? ORDER BY year, semester, sec_id");
...
if(isset($_POST['register'])){
    ...
    $stmt->execute();
    echo "<p style='color:green;'>Class registered successfully!</p>";
}

// Fetch all distinct course IDs
$courses = $conn->query("SELECT DISTINCT course_id FROM section ORDER BY course_id");
$course_list = [];
...
<form method="POST" id="registerForm" style="display:flex; gap:10px; align-items:center;">
    <label>Course:
...
document.getElementById('course_id').addEventListener('change', function() {
    ...
    fetch(`register.php?mode=sections&course_id=${encodeURIComponent(course_id)}`)
